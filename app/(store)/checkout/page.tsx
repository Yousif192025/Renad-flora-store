...
// تعريف الـ Schema مع التحقق الصحيح
...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    if (!selectedPayment) {
      toast.error("اختاري طريقة الدفع");
      return;
    }
    ...
    try {
      ...
      const { data: order, error } = await supabase
        ...
        .insert([{
          ...
          shipping_info: {
            full_name: shippingAddress.full_name || customerInfo.full_name,
            ...
            <div key={s} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                  idx < currentIdx ? "bg-pink-500 text-white" :
                  idx === currentIdx ? "bg-pink-600 text-white ring-4 ring-pink-100" :
                  "bg-gray-100 text-gray-400"
                }`}>
                  {idx < currentIdx ? "✓" : idx + 1}
                </div>
                <span className={`text-xs mt-1.5 font-medium ${
                  idx === currentIdx ? "text-pink-600" :
                  idx < currentIdx ? "text-pink-400" :
                  "text-gray-400"
                }`}>
                  {STEP_LABELS[s]}
                </span>
              ...
              <form onSubmit={customerForm.handleSubmit(onCustomerSubmit)} className="space-y-5">
                ...
                <div>
                  ...
                  {customerForm.formState.errors.email && (
                    <p className="text-red-500 text-xs mt-1">بريد غير صحيح</p>
                  )}
                </div>
                ...
                <button type="submit" className="btn-flora w-full py-4 text-base">
                  التالي: عنوان الشحن ←
                </button>
              </form>
            )}
            ...
            {step === "shipping" && (
              <form onSubmit={shippingForm.handleSubmit(onShippingSubmit)} className="space-y-5">
                ...
                <div>
                  ...
                  <select {...shippingForm.register("city")} className="input-flora">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      {...shippingForm.register("district")}
                ...
                {total >= 100 && (
                  <div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      {INSTALLMENT_PROVIDERS.filter(
                        (p) => total >= p.minAmount && total <= p.maxAmount
                      ).map((p) => (
                        <button
                          key={p.id}
                          type="button"
                        ...
                        >
                          ...
                          <p className="text-pink-600 font-bold">
                            {formatPrice(Math.ceil(total / p.months))} / شهر
                          </p>
                          <p className="text-xs text-gray-400">
                            {p.months} دفعات بدون فوائد
                          </p>
                        </button>
                      ))}
                    </div>
            ...
            {step === "review" && (
              <div className="space-y-5">
                ...
                <div className="bg-pink-50 rounded-2xl p-4">
                  ...
                  <p className="text-sm text-gray-600">
                    {PAYMENT_METHODS.find((m) => m.id === selectedPayment)?.nameAr ??
                      INSTALLMENT_PROVIDERS.find((p) => p.id === selectedPayment)?.nameAr ??
                      selectedPayment}
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {placingOrder ? "جاري تقديم الطلب..." : "تأكيد الطلب ✓"}
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3 mb-5 max-h-60 overflow-y-auto no-scrollbar">
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-pink-50 shrink-0 flex items-center justify-center text-2xl">
